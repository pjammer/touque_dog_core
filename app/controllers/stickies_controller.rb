class StickiesController < ApplicationController
  ...
  def update
    ...
    respond_to do |format|
      if @stickie.update_attributes(params[:stickie])
        flash[:notice] = 'Stickie was successfully updated.'
        format.html { redirect_to(@user) }
        format.xml  { head :ok }
      else
        format.html { render :action => "edit" }
        format.xml  { render :xml => @stickie.errors, :status => :unprocessable_entity }
      end
    end
  end
