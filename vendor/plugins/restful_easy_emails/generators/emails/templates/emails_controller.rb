class EmailsController < ApplicationController
  ...
  # GET /emails/1
  def show
    @email = Message.find_by_id(params[:id])
  ...
  # POST /emails
  def create
    @email = Message.new((params[:email] || {}).merge(:sender => rezm_user))
  ...
  def inbox
    ...
    @emails = rezm_user.inbox_profile_emails
  ...
  def destroy_selected
    ...
    respond_to do |format|
      if !params[:to_delete].nil?
        emails = params[:to_delete].map { |m| Message.find_by_id(m) } 
